...
describe 'Registrations API', type: :request do
  describe '#Post v1/registrations' do
    let(:valid_user_params) { get_json(resource: 'user', filename: 'valid_registration_params') }

    let(:invalid_user_params) { get_json(resource: 'user', filename: 'invalid_registration_params') }
